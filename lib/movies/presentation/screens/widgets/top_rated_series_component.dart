...
class TopRatedSeriesComponent extends StatelessWidget {
  const TopRatedSeriesComponent({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<SeriesBloc, SeriesState>(
        buildWhen: (previous, current) =>
            previous.topRatedSeriesState != current.topRatedSeriesState,
        builder: (context, state) {
          switch (state.topRatedSeriesState) {
              ...
              return FadeIn(
                ...
                child: SizedBox(
                  ...
                  child: ListAnimatorData(
                    ...
                    itemCount: state.getTopRatedSeries.length,
                    itemBuilder: (context, index) {
                      final series = state.getTopRatedSeries[index];
                      return Container(
            ...
            case RequestState.error:
              return SizedBox(
                  height: 170.0.h,
                  child: Center(child: Text(state.topRatedSeriesMessage)));
          }
        });
  }
}
